...
int main(int argc, char** argv) {
...

    {
        ...
        for (int iter = 0; iter < benchmarkingIters; ++iter) {
            s = 0;
            res_gpu.write(&s, sizeof(as[0]));
            sum.exec(gpu::WorkSize(workGroupSize, global_work_size), as_gpu, n, res_gpu);
            res_gpu.read(&s, sizeof(unsigned int));
